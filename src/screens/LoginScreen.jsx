...
function Spinner() {
  return (
    <div style={{ width:16, height:16, border:`2px solid #00000040`, borderTop:`2px solid #000`, borderRadius:"50%", animation:"spin 0.8s linear infinite", display:"inline-block", marginRight:8, verticalAlign:"middle" }} />
  );
}
...
export default function LoginScreen({ onLogin }) {
  ...
  const ensureWalletExists = async (userData) => {
    try {
    ...
    } catch (err) { console.error("Wallet creation failed:", err); }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setLoading(false);
      triggerError(friendlyError(err.code));
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Build wallet doc — include partnerCode if signed up via partner link
      const walletData = {
        uid:        result.user.uid,
        name:       name.trim(),
  ...
  const cardStyle = {
    ...
    position:"relative", zIndex:1, animation:"fadeSlideUp 0.5s ease both",
  };

  const Logo = ({ sub }) => (
    <div style={{ textAlign:"center", marginBottom:28 }}>
      <div style={{ fontSize:52, fontWeight:900, letterSpacing:"-0.02em", lineHeight:1, background:`linear-gradient(135deg,${C.gold},${C.goldLight},${C.gold})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
        NOVA
      </div>
  ...
  const ErrorBox = () => error ? (
    <div style={{ marginTop:14, padding:"12px 16px", borderRadius:10, background:`${C.red}15`, border:`1px solid ${C.red}40`, color:C.red, fontSize:13, textAlign:"center", fontWeight:600, width:"100%" }}>
      {error}
    </div>
  ) : null;
  ...
  if (screen === "forgot") return (
    <div style={pageStyle}>
      ...
      <div style={{ ...cardStyle, gap:20 }}>
        ...
        <ErrorBox />
        {!resetSent && (
          <span onClick={()=>{ setScreen("login"); reset(); }} style={{ fontSize:12, color:C.gold, cursor:"pointer", marginTop:8 }}>
            ← Back to Login
          </span>
        )}
      </div>
  ...
  if (screen === "register") return (
    <div style={pageStyle}>
      ...
      <div style={{ ...cardStyle, gap:0 }}>
        ...
        {/* Partner notice */}
        {partnerRef && (
          <div style={{ width:"100%", marginBottom:16, padding:"10px 14px", borderRadius:10, background:`${C.gold}10`, border:`1px solid ${C.gold}30`, display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:14 }}>🤝</span>
            <div>
              <div style={{ fontSize:12, fontWeight:700, color:C.gold }}>Invited by {partnerRef.name}</div>
              <div style={{ fontSize:11, color:C.muted }}>Special fee rates will apply to your account</div>
            </div>
          </div>
        )}
        ...
        <div style={{ width:"100%", display:"flex", flexDirection:"column", gap:14, animation:shake?"shake 0.4s ease":"none" }}>
          ...
          <div style={{ background:C.bgElevated, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px 16px" }}>
            <div style={{ fontSize:11, color:C.muted, lineHeight:1.6 }}>
              By creating an account you agree to our <span style={{ color:C.gold, cursor:"pointer" }}>Terms of Service</span> and <span style={{ color:C.gold, cursor:"pointer" }}>Privacy Policy</span>
            </div>
          </div>
        ...
        <div style={{ marginTop:20, fontSize:13, color:C.muted }}>
          ...
          <span onClick={()=>{ setScreen("login"); reset(); setEmail(""); setPassword(""); }} style={{ color:C.gold, cursor:"pointer", fontWeight:700 }}>
            Sign In
          </span>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ width:"100%", display:"flex", flexDirection:"column", gap:14, animation:shake?"shake 0.4s ease":"none" }}>
          ...
          <div style={{ textAlign:"right", marginTop:-4 }}>
            <span onClick={()=>{ setScreen("forgot"); reset(); }} style={{ fontSize:12, color:C.gold, cursor:"pointer", fontWeight:600 }}>
              Forgot password?
            </span>
          </div>
          ...
          <div style={{ textAlign:"center", fontSize:13, color:C.muted }}>
            ...
            <span onClick={()=>{ setScreen("register"); reset(); setEmail(""); setPassword(""); }} style={{ color:C.gold, cursor:"pointer", fontWeight:700 }}>
              Create Account
            </span>
          </div>
          <ErrorBox />
        </div>
        <div style={{ marginTop:36, fontSize:10, color:C.muted, letterSpacing:"0.12em" }}>NOVA VAULT · SECURED BY 256-BIT AES</div>
      </div>
      <CSS />
    </div>
  );
