  ...
  return PARTICLES.map(p => (
    <div
      ...
      style={{
        ...
        opacity:0.15,
        pointerEvents:"none",
        animation:`float-p ${p.dur}s ease-in-out ${p.delay}s infinite alternate`
      }}
    />
  ));
}
...
const friendlyError = (code) => {
  switch(code) {
    ...
    case "auth/invalid-credential":
      return "Incorrect email or password. Check and try again.";
...
export default function LoginScreen({ onLogin }) {
  ...
  const [referralCode, setReferralCode] = useState(() => {
    try {
      ...
      if (fromUrl) {
        ...
        localStorage.setItem("novaReferralCode", code);
        return code;
      }
      return localStorage.getItem("novaReferralCode") || "";
    } catch { return ""; }
  });
  ...
  // CREATE / FIX WALLET FOR ALL USERS
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // FIX OLD USERS WHO DON'T HAVE WALLET
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Create the user profile. Referral links attach the new user to
      // the partner admin who owns the referral code.
      let assignedAdminId = null;
      let assignedReferralCode = null;

      if (referralCode.trim()) {
        const normalizedCode = referralCode.trim().toUpperCase();
        const referralSnap = await getDoc(doc(db, "referralLinks", normalizedCode));
        if (!referralSnap.exists()) {
          throw new Error("This partner registration link is invalid or expired.");
        }
        const referral = referralSnap.data();
        if (referral.active === false || !referral.adminId) {
          throw new Error("This partner registration link is inactive.");
        }
        assignedAdminId = referral.adminId;
        assignedReferralCode = referral.code || normalizedCode;
      }
      ...
      localStorage.setItem(
        "novaUser",
        JSON.stringify({ ...user, role:"user", adminId: assignedAdminId })
      );
      localStorage.removeItem("novaReferralCode");
  ...
  const handleForgot = async () => {
    if (!resetEmail) {
      triggerError("Enter your email address.");
      return;
    }
    ...
    <div style={{ textAlign:"center", marginBottom:28 }}>
      <style>{`
        @keyframes wordmarkSettle {
          from { opacity: 0; letter-spacing: 0.35em; transform: translateY(10px) scale(0.94); filter: blur(4px); }
          to   { opacity: 1; letter-spacing: -0.02em; transform: translateY(0) scale(1); filter: blur(0); }
        }
        @keyframes wordmarkShimmer {
          from { background-position: -160% 0; }
          to   { background-position: 260% 0; }
        }
        @keyframes wordmarkSubFade {
      ...
      <div
        style={{
          ...
          backgroundClip:"text",
          animation:"wordmarkSettle 0.9s cubic-bezier(0.16,1,0.3,1) both, wordmarkShimmer 4.5s ease-in-out 1.1s infinite",
        }}
      >
        NOVA
      </div>
      ...
      <div
        style={{
          ...
          marginTop:2,
          opacity:0,
          animation:"wordmarkSubFade 0.7s ease 0.35s both",
        }}
      >
        {sub}
      </div>
